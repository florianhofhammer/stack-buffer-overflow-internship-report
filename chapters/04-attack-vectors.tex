...
\todo{Maybe rework section to also include overwriting of \texttt{.fini\_\linebreak[0]array} data $\Rightarrow$ targeted overwriting instead of just \acs{got} hijacking}
...
This approach of guessing a stack value byte for byte can be extended to other values than the stack canary as well.
...
The need for the vulnerability being exploitable by a function working on binary data such as \texttt{read} or \texttt{memcpy} is also present when looking at attacks based on \crtlnameref{subsec:bf-information-leaking}.
...
\begin{table}[ht]
	...
	\makebox[\textwidth][c]{ % command is necessary in order to center the table correctly
		...
		\setlength{\colwidth}{1.8cm}
		\begin{tabular}{|l|*{6}{P{\colwidth}|}}
			\hline
			Attack approach	& \multicolumn{6}{@{}l@{}}{
								\begin{tabular}{*{6}{p{\colwidth}|}}
									\multicolumn{6}{l|}{Security measure}	\\ \hline
									\texttt{\_FORTIFY\linebreak[0]\_SOURCE}	& \acs{dep} / \acs{nx} bit	& \acs{aslr} (without \acs{pie} / \acs{pic})	& \acs{aslr} (with \acs{pie} / \acs{pic})	& Stack canaries	& (full) \acs{relro}	\\
								\end{tabular}					
							  } \\ \hline
			Code injection:					&		 &		  &		   &		&		 &		  \\
...
\end{table}
\todo{Fix awful indentation issue $\Rightarrow$ hspace in table shouldn't be necessary, maybe put table onto landscape page}

The main overview is provided in \cref{tab:attack-comparison}.
This table shows for the previously presented attack approaches which security measures they can bypass (\cmark) and which security measures can prevent those attacks (\xmark).
...
However, for the exploit to work as described, it is necessary to have a stack buffer overflow which allows an attacker to overwrite a string pointer and to have a write operation writing user-controlled data and working on the overwritten pointer later in the program.
...
%	}
%	\item{
%		\gls{aslr} brute-force $\Rightarrow$ on \texttt{x86\_64} difficult because of too much entropy \cmark
%	}
%	\item{
